...
def _record(mid: int, author: str, text: str, *, reply: int | None = None, order: int | None = None) -> MessageRecord:
    return MessageRecord(
        ...
        source_file="گروه دکتر جوان/messages10.html",
        source_page=10,
        source_order=order if order is not None else mid,
        ...
        source_locator=f"گروه دکتر جوان/messages10.html#go_to_message{mid}",
    )


def _candidate(mid: int, author: str, text: str, *, score: float = 6.0, reply: int | None = None, context=()) -> EvidenceCandidate:
    return EvidenceCandidate(
        message=_record(mid, author, text, reply=reply),
        local_score=score,
...
def _plan(*, searchable=True, families=None):
    return json.dumps({
        ...
        "aliases": ["composite"],
        "optional_concepts": ["تجربه", "پیشنهاد"],
        ...
        "phrases": [],
        "exclude_terms": [],
        "low_information_terms": ["کدوم", "خوبه"],
        "reply_context": True,
    }, ensure_ascii=False)
...
def test_planner_cache_is_bound_to_index_fingerprint(tmp_path: Path):
    ...
    first = service.answer("کامپوزیت")
    second = service.answer("کامپوزیت")
...
def test_malformed_planner_fallback_is_not_cached(tmp_path: Path):
    ...
    assert [call["request_type"] for call in provider.calls] == ["search_plan", "synthesis", "search_plan", "synthesis"]
...
def test_multi_query_fusion_preserves_reply_context_and_diversifies_authors():
    ...
    plan = SearchPlan(
        ...
        query_families=(SearchFamily("topic", ("کامپوزیت",)), SearchFamily("product", ("ProductQ",))),
        phrases=(),
        exclude_terms=(),
...
def test_post_filter_duplicate_queries_do_not_fake_family_coverage():
    ...
    plan = SearchPlan(
        ...
        core_concepts=("کامپوزیت",),
        aliases=(),
        optional_concepts=("خوب",),
        entity_types=("product",),
        query_families=(SearchFamily("topic", ("کامپوزیت",)), SearchFamily("quality", ("کامپوزیت خوب",))),
        phrases=(),
        exclude_terms=(),
        low_information_terms=("خوب",),
        reply_context=True,
    )
    report = retrieve_with_plan(backend, plan)

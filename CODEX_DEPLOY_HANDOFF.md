...
4. **Do not source `/etc/drjavanbot/drjavanbot.env` before bootstrap.** A legacy installation may still contain the historical unquoted Persian archive path. Pre-bootstrap pytest does not need runtime secrets/env.
5. Run `pytest -q` directly from the repository environment. If any test fails, stop and report exact command/exit code/trace; do not modify code.
6. Run `sudo python3 deploy/bootstrap_self_update.py`. Bootstrap repairs/normalizes the archive env entry at the beginning of its execution, before release tests.
7. Verify only:
...
8. Only after successful bootstrap, source `/etc/drjavanbot/drjavanbot.env` for manual smoke and run:
   - `/opt/drjavanbot/current/.venv/bin/drjavanbot-smoke`
   - `/opt/drjavanbot/current/.venv/bin/drjavanbot-smoke --telegram`
9. Send one normal Telegram question and verify the internal-error regression is gone.
10. Do not request, print, move or replace the AvalAI key; existing runtime secret is preserved.
11. Do not stop/restart/modify any other Telegram bot on the server.
